...
@section('content')
    <div class="editClient-container">
        ...
        <div class="head-title">
            ...
            <p>Müştəri məlumatı düzəltiş et</p>
        </div>
        <form action="{{ route('admin.customer.update', $customer->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="form-line">
                <div class="form-item">
                    ...
                    <!-- Avotmatik yaranir -->
                    <input type="text" disabled value="{{ $customer->id }}" placeholder="text here">
                </div>
                @if ($customer->type == \App\Enums\CustomerType::LEGAL)
                    <div class="form-item">
                        ...
                        <input type="text" name="company_name" oninput="convert_letter(this)"
                            value="{{ $customer->company_name }}" placeholder="text here">
                    </div>
                    <div class="form-item">
                        <label for="">
                            Direktor/ Ad soyad
                        </label>
                        <input type="text" name="name" oninput="convert_letter(this)" value="{{ $customer->name }}"
                ...
                <div class="form-item">
                    ...
                    <select name="gender" id="" class="nice-select">
                        <option value="male" {{ $customer->gender == \App\Enums\Gender::MALE ? 'selected' : '' }}>Kişi
                        </option>
                        <option value="female" {{ $customer->gender == \App\Enums\Gender::FEMALE ? 'selected' : '' }}>Qadın
                        </option>
                    </select>
                ...
                @if ($customer->type == \App\Enums\CustomerType::PHYSICAL)
                    <div class="form-item">
                        ...
                        <input type="text" name="fin" value="{{ $customer->fin }}"
                            oninput="convert_alphanumeric(this)" placeholder="text here">
                    </div>
                    <div class="form-item">
                        <label for="">
                        ...
                        <input type="text" name="serial_number" oninput="convert_alphanumeric(this)"
                            value="{{ $customer->serial_number }}" placeholder="text here">
                    </div>
                @endif
                <div class="form-item">
                    <label for="">
                    ...
                    <input type="text" name="voen" oninput="convert_numeric(this)"
                        value="{{ old('voen', $customer->voen) }}" placeholder="text here">
                </div>
                <div class="form-item">
                    <label for="">
                        Əlaqə nömrəsi
                    </label>
                    <input type="text" name="phone" value="{{ $customer->phone }}" oninput="convert_numeric(this)"
                        placeholder="text here">
                </div>
                <div class="form-item">
                    <label for="">
                    ...
                    <select name="source_id" id="" class="nice-select">
                        ...
                        @foreach ($sources as $source)
                            <option value="{{ $source->id }}"
                                {{ $customer->source_id == $source->id ? 'selected' : '' }}>
                                {{ $source->name }}</option>
                        @endforeach
                    </select>
                    <button class="resetSelectBtn" type="button">
                ...
                <div class="form-item">
                    ...
                    <input type="text" class="datetimepicker" name="contract_start_date"
                        value="{{ $customer->contract_start_date?->format('d.m.Y') }}" autocomplete="off"
                        placeholder="Tarix">
                ...
                <div class="form-item">
                    ...
                    <input type="text" autocomplete="off" name="contract_end_date"
                        value="{{ $customer->contract_end_date?->format('d.m.Y') }}" class="datetimepicker"
                        placeholder="Tarix">
                    <div class="calendar-icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="form-item">
                    ...
                    <input type="text" autocomplete="off" name="date"
                        value="{{ $customer->date?->format('d.m.Y') }}" class="datetimepicker" placeholder="Tarix">
                    <div class="calendar-icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                ...
                <div class="form-item">
                    ...
                    <select name="type" onchange="change_customer_type(this)" id="" class="nice-select">
                        <option value="physical"
                            {{ $customer->type == \App\Enums\CustomerType::PHYSICAL ? 'selected' : '' }}>
                            Fiziki səxs
                        </option>
                        <option value="legal" {{ $customer->type == \App\Enums\CustomerType::LEGAL ? 'selected' : '' }}>
                            Korporativ</option>
                        {{-- <option value="owner" {{ $customer->type == \App\Enums\CustomerType::OWNER ? 'selected' : '' }}>
                            Fərdi sahibkar</option> --}}
                    </select>
                    <button class="resetSelectBtn" type="button">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <select name="payment_term_id" id="" class="nice-select">
                        ...
                        @foreach ($payment_terms as $payment_term)
                            <option value="{{ $payment_term->id }}"
                                {{ old('payment_term_id', $customer->payment_term_id) == $payment_term->id ? 'selected' : '' }}>
            ...
            <div class="form-note">
                ...
                <textarea name="note" id="" placeholder="Qeyd">{{ $customer->note }}</textarea>
            </div>
            <div class="edit_clientFile-container">
                <div class="edit_clientFile-box">
                    <div class="icon">
                    ...
                    <div class="edit_clientFile-body">
                ...
                <div class="edit_clientFile-fileUpload">
                    @foreach ($customer->files as $customer_file)
                        <div class="edit_clientFile-fileUpload-area">
                            <img src="{{ asset('back/assets') }}/images/pdfImg.svg" alt="">
                            <div class="edit_clientFile-fileUpload-main">
                                <div class="edit_clientFile-fileUpload-top">
                                    <span
                                        class="edit_clientFile-FileName">{{ isset(explode('/', $customer_file->file)[2]) ? explode('/', $customer_file->file)[2] : '' }}</span>
                                    <p class="edit_clientFile-fileSize">10 MB</p>
                                </div>
                                <div class="edit_clientFile-fileProgress">
                                    <div class="uploadLine" style="width: 100%;"></div>
                                </div>
                            </div>
                            <button onclick="delete_file({{ $customer_file->id }})" class="removeEditClientFile"
                                type="button">
            ...
            @include('back.pages.customer.section.property', ['customer' => $customer])
            <button class="submitEditClient" type="submit">Yadda saxla</button>
        </form>
    </div>
@endsection
...
@push('js')
    ...
    <script>
        document.querySelectorAll(".edit_clientFile-container").forEach((container) => {
            const edit_clientFileInput = container.querySelector('input[type="file"]');
            const edit_clientFileUpload = container.querySelector(".edit_clientFile-fileUpload");

            container.querySelectorAll(".removeEditClientFile").forEach((item) => {
                item.addEventListener("click", () => {
                    item.parentElement.remove();
                });
            });

            edit_clientFileInput?.addEventListener("change", async () => {
                if (edit_clientFileInput.files.length > 0) {
                    [...edit_clientFileInput.files].forEach(async (file) => {
                        const edit_clientFileUploadArea = document.createElement("div");
                        edit_clientFileUploadArea.className =
                            "edit_clientFile-fileUpload-area";

                        edit_clientFileUploadArea.innerHTML = `
                  <img src="{{ asset('back/assets') }}/images/pdfImg.svg" alt="">
                  <div class="edit_clientFile-fileUpload-main">
                      <div class="edit_clientFile-fileUpload-top">
                          <span class="edit_clientFile-FileName">${file.name}</span>
                          <p class="edit_clientFile-fileSize">${(
                            file.size /
                            (1024 * 1024)
                          ).toFixed(2)} MB</p>
                      </div>
                  ...
                  <button class="removeEditClientFile" type="button">
              ...
              `;
                        ...
                        edit_clientFileUpload.appendChild(edit_clientFileUploadArea);

                        const edit_clientFile_uploadLine = edit_clientFileUploadArea
                            .querySelector(".uploadLine");
                        edit_clientFile_uploadLine.style.transition = "none";
                        edit_clientFile_uploadLine.style.width = "0%";

                        await new Promise((resolve) => setTimeout(resolve, 100));

                        edit_clientFile_uploadLine.style.transition = "width 0.5s linear";
                        simulateFileUploadProgress(edit_clientFile_uploadLine);

                        // Yeni eklenen butona silme eventini ekle
                        const removeButton = edit_clientFileUploadArea.querySelector(
                            ".removeEditClientFile");
                        removeButton.addEventListener("click", () => {
                            edit_clientFileUploadArea.remove();
                        });
                    });
                }
                // edit_clientFileInput.value = ""; // Aynı dosyayı tekrar seçebilmek için input temizlendi
            });
        });
