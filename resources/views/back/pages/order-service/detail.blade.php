...
@section('content')
    {{-- @switch($order_service->service_type)
        @case(\App\Enums\ServiceType::DEPO) --}}
    <div class="serviceStorageView-container">
        <a href="{{ route('admin.order-service.index', ['order_item_id' => $order_service->order_item_id]) }}"
            class="backLink">
        ...
        <div class="serviceStorageView-box">
            <div class="serviceStorageView-box-item">
                <div class="item">
                    <h2>Xidmət növü</h2>
                    <p>{{ $order_service->service->name }}</p>
                </div>
                @if (!is_null($order_service->document_type))
                    <div class="item">
                        <h2>Sənəd növü</h2>
                        <p>{{ $order_service->document_type }}</p>
                    </div>
                @endif
                <div class="item">
                    <h2>Xərc növü</h2>
                    <p>{{ $order_service->expense_type?->name }}</p>
                </div>
                <div class="item">
                    <h2>Vendor</h2>
            ...
            <div class="serviceStorageView-box-item">
                ...
                <div class="item">
                    <h2>Start date</h2>
                    <p>{{ $order_service->start_date?->format('d.m.Y') }}</p>
                </div>
                <div class="item">
                    <h2>End date</h2>
                    <p>{{ $order_service->end_date?->format('d.m.Y') }}</p>
                </div>
                <div class="item">
                    <h2>Son ödəmə tarixi</h2>
                    <p>{{ $order_service->last_payment_date?->format('d.m.Y') }}</p>
                </div>
            </div>
        </div>
        <div class="note-box">
