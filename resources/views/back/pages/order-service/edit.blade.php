...
@section('content')
    <div class="service-add-container">
        ...
        <div class="head-title">
            ...
            <p>Xidmət redaktə et</p>
        </div>
        <form action="{{ route('admin.order-service.update', $order_service->id) }}" method="POST"
                ...
                <div class="service-form">
                    <div class="form-line">
                        <div class="form-item">
                            ...
                            <select name="service_id" onchange="get_inputs()" id="" class="nice-select">
                                <option value="">Seçin</option>
                                @foreach ($services as $service)
                                    <option value="{{ $service->id }}"
                                        {{ $order_service->service_id == $service->id ? 'selected' : '' }}>
                                        {{ $service->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-item selectable">
                            ...
                            <select name="document_type" id="" class="nice-select">
                                <option value="">Seçin</option>
                                @foreach ($document_types as $document_type)
                                    <option value="{{ $document_type->id }}"
                                        {{ $order_service->document_type == $document_type->name ? 'selected' : '' }}>
                        ...
                        <div class="form-item selectable">
                            ...
                            <select name="expense_type" onchange="get_expense_type(this)" id=""
                                class="nice-select">
                                <option value="">Seçin</option>
                                @foreach ($expense_types as $expense_type)
                                    <option value="{{ $expense_type->id }}"
                                        {{ $order_service->expense_type_id == $expense_type->id ? 'selected' : '' }}>
                            ...
                            <select name="vendor_id" id="" class="nice-select">
                                ...
                                @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->id }}"
                                        {{ $order_service->vendor_id == $vendor->id ? 'selected' : '' }}>
                        ...
                        <div class="form-item" style="display: flex; gap:20px;">
                            <div class="form-input" style="width: 75%;">
                                ...
                                <input type="text" name="purchase_price" oninput="convert_numeric(this)"
                                    value="{{ old('purchase_price', $order_service->purchase_price) }}"
                                ...
                                <select name="purchase_currency" id="" class="nice-select">
                                    @foreach ($currencies as $currency)
                                        <option value="{{ $currency->symbol }}"
                                            {{ $order_service->purchase_currency == $currency->symbol }}>
                        ...
                        <div class="form-item">
                            ...
                            <input type="text" autocomplete="off" name="date"
                                value="{{ $order_service->date?->format('d.m.Y') }}" class="datetimepicker"
                                placeholder="Tarix">
                        ...
                        <div class="form-item" style="display:flex; gap: 20px;">
                            <div class="form-input" style="width: 75%;">
                                ...
                                <input type="text" name="sale_price" oninput="convert_numeric(this)"
                                    value="{{ old('sale_price', $order_service->sale_price) }}" placeholder="text here">
                            </div>
                            <div class="form-select">
                                <select name="sale_currency" id="" class="nice-select">
                                    @foreach ($currencies as $currency)
                                        <option value="{{ $currency->symbol }}"
                                            {{ $order_service->sale_currency == $currency->symbol ? 'selected' : '' }}>
                                            {{ $currency->code }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-item">
                            ...
                            <input type="text" autocomplete="off" name="start_date"
                                value="{{ $order_service->start_date?->format('d.m.Y') }}" class="datetimepicker"
                                placeholder="Tarix">
                            <div class="calendar-icon">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="form-item selectable">
                            ...
                            <input type="text" name="end_date"
                                value="{{ $order_service->end_date?->format('d.m.Y') }}" autocomplete="off"
                                class="datetimepicker" placeholder="Tarix">
                            <div class="calendar-icon">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                        ...
                        <div class="form-item selectable">
                            ...
                            <input type="text" name="last_payment_date"
                                value="{{ $order_service->last_payment_date?->format('d.m.Y') }}" autocomplete="off"
                                class="datetimepicker" placeholder="Tarix">
                            <div class="calendar-icon">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="form-note selectable depo documentation">
                        <label for="">
                            Qeyd
                        </label>
                        <textarea name="note" id="" placeholder="Qeyd">{{ $order_service->note }}</textarea>
                    </div>
                </div>
            </div>
...
@push('js')
    ...
    <script>
        ...
        function get_inputs() {
            let item = document.querySelector('[name="service_id"]');
            let id = item.value;
