    <div class="form-item" style="display: flex; gap: 10px; width: 500px;margin-bottom:20px">
        <div class="form-item">
            <label for="">En (M)</label>
            <input type="text" oninput="calculate_total_cbm(this)"
                value="{{ isset($order_size) ? $order_size->width : '' }}" name="width[]" placeholder="En...">
        </div>
        <div class="form-item">
            <label for="">Uzunluq (M)</label>
            <input type="text" oninput="calculate_total_cbm(this)" name="length[]"
                value="{{ isset($order_size) ? $order_size->length : '' }}" placeholder="Uzunluq...">
        </div>
        <div class="form-item">
            <label for="">Hündürlük (M)</label>
            <input type="text" oninput="calculate_total_cbm(this)" name="height[]"
                value="{{ isset($order_size) ? $order_size->height : '' }}" placeholder="Hündürlük...">
        </div>
        @if ($counter != 0)
            <button type="button" onclick="remove_size(this)" class="remove-size">
                Sil
