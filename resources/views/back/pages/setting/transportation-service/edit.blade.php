...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Daşınma xidməti redaktə et</h2>
        <form method="POST" action="{{ route('admin.transportation-service.update', $transportation_service->id) }}"
            ...
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $transportation_service->name }}"
                    ...
                    <select name="transportation_type_id" id="">
                        ...
                        @foreach ($transportation_types as $transportation_type)
                            <option value="{{ $transportation_type->id }}"
                                {{ $transportation_service->transportation_type_id == $transportation_type->id ? 'selected' : '' }}>
                ...
                <div class="form-item">
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $transportation_service->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $transportation_service->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
