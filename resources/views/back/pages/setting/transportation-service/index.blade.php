...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">Daşınma xidmətləri</h2>
        <a href="{{ route('admin.transportation-service.create') }}" class="addSettingLink">
                ...
                <table id="sortable-table">
                    ...
                    <tbody>
                        @foreach ($transportation_services as $transportation_service)
                            <tr data-id="{{ $transportation_service->id }}">
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $transportation_service->name }}</td>
                                <td>{{ $transportation_service->transporation_type->name }}</td>
                                <td>
                                    <label class="switch">
                                        <input type="checkbox" {{ $transportation_service->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $transportation_service->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation">
                                        ...
                                        <div class="settingTable-operation-links">
                                            <a href="{{ route('admin.transportation-service.edit', $transportation_service->id) }}"
                                            ...
                                            <button
                                                data-href="{{ route('admin.transportation-service.destroy', $transportation_service->id) }}"
