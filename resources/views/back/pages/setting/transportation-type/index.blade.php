...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">Daşınma növləri</h2>
        <a href="{{ route('admin.transportation-type.create') }}" class="addSettingLink">
                ...
                <table id="sortable-table">
                    ...
                    <tbody>
                        @foreach ($transportation_types as $transportation_type)
                            <tr data-id="{{ $transportation_type->id }}">
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $transportation_type->name }}</td>
                                <td>
                                    <label class="switch">
                                        <input type="checkbox" {{ $transportation_type->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $transportation_type->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation">
                                        ...
                                        <div class="settingTable-operation-links">
                                            <a href="{{ route('admin.transportation-type.edit', $transportation_type->id) }}"
                                            ...
                                            <button
                                                data-href="{{ route('admin.transportation-type.destroy', $transportation_type->id) }}"
