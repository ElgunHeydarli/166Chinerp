...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Daşınma növü redaktə et</h2>
        <form method="POST" action="{{ route('admin.transportation-type.update', $transportation_type->id) }}"
            ...
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $transportation_type->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $transportation_type->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $transportation_type->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
