...
@section('content')
    <div class="create-draftOrder-container">
        <form method="POST" action="{{ route('admin.car-type.update', $car_type->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $car_type->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $car_type->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $car_type->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
