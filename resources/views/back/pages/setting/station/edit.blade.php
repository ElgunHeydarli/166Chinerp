...
@section('content')
    <div class="create-draftOrder-container">
        <form method="POST" action="{{ route('admin.station.update', $station->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $station->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $station->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $station->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
