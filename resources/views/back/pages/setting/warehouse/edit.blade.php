...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Anbar redaktə et</h2>
        <form method="POST" action="{{ route('admin.warehouse.update', $warehouse->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $warehouse->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $warehouse->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $warehouse->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
