...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">Valyutalar</h2>
        <a href="{{ route('admin.currency.create') }}" class="addSettingLink">
                    ...
                    <tbody>
                        @foreach ($currencies as $currency)
                            <tr data-id="{{ $currency->id }}">
                                ...
                                <td>{{ $currency->exchange?->index ?? 1 }}</td>
                                <td>
                                    <label class="switch">
                                        <input onchange="change_status(this,{{ $currency->id }})" type="checkbox"
                                            {{ $currency->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $currency->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation" onclick="get_active(this)">
                                        ...
                                        <div class="settingTable-operation-links">
                                            ...
                                            <button data-href="{{ route('admin.currency.destroy', $currency->id) }}"
...
@push('js')
    ...
    <script>
        ...
        function change_status(item, id) {
            let url = `/currency/${id}/change-status`;
