...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">İcazələr</h2>
        <div class="tabContent-head">
            <select id="group_name" onchange="get_group_name(this)" class="nice-select">
                <option value="">Qrup</option>
                @foreach ($group_names as $group_name)
                    <option value="{{ $group_name['group_name'] }}"
                        {{ request('group_name') == $group_name['group_name'] ? 'selected' : '' }}>
            ...
            <a href="{{ route('admin.permission.create') }}" class="addSettingLink">
        ...
        <div class="settingTable">
            <div class="settingTable-table">
                    ...
                    <tbody>
                        @foreach ($permissions as $permission)
                            <tr data-id="{{ $permission->id }}">
                                ...
                                <td>
                                    <label class="switch">
                                        <input onchange="change_status(this,{{ $permission->id }},'permission')"
                                            type="checkbox" {{ $permission->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $permission->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation">
                                        <button class="settingTable-operation-btn" onclick="get_active(this)"
                                            type="button">
                                            Əməliyyat
                                        ...
                                        <div class="settingTable-operation-links">
                                            ...
                                            <button data-href="{{ route('admin.permission.destroy', $permission->id) }}"
                ...
                {{ $permissions->withQueryString()->links('back.section.pagination') }}
            </div>
...
@push('js')
    <script>
        sort('permission');
        function get_group_name(item) {
            let group_name = item.value;
            document.querySelector('[name="group_name"]').value = group_name;
        };
