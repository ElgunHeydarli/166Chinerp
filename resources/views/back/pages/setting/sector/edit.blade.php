...
@section('content')
    <div class="create-draftOrder-container">
        <form method="POST" action="{{ route('admin.sector.update', $sector->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $sector->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $sector->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $sector->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
