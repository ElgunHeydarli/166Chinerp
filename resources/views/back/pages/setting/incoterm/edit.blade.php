...
@section('content')
    <div class="create-draftOrder-container">
        <form method="POST" action="{{ route('admin.incoterm.update', $incoterm->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $incoterm->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $incoterm->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $incoterm->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
