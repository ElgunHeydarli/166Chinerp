...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Rayon redaktə et</h2>
        <form method="POST" action="{{ route('admin.district.update', $district->id) }}"
            ...
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $district->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    ...
                    <select name="city_id" id="">
                        ...
                        @foreach ($cities as $city)
                            <option
                                value="{{ $city->id }}" {{ $district->city_id == $city->id ? 'selected' : '' }}>
                                {{ $city->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-item">
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $district->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $district->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
