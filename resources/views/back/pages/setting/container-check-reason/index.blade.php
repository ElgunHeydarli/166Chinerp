...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">Konteyner yoxlama səbəbləri</h2>
        <div class="tabContent-head">
            <a href="{{ route('admin.container-check-reason.create') }}" class="addSettingLink">
                ...
                <table id="sortable-table">
                    ...
                    <tbody>
                        @foreach ($container_check_reasons as $container_check_reason)
                            <tr data-id="{{ $container_check_reason->id }}">
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $container_check_reason->name }}</td>

                                <td>
                                    <label class="switch">
                                        <input type="checkbox" {{ $container_check_reason->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $container_check_reason->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation">
                                        ...
                                        <div class="settingTable-operation-links">
                                            <a href="{{ route('admin.container-check-reason.edit', $container_check_reason->id) }}"
                                            ...
                                            <button
                                                data-href="{{ route('admin.container-check-reason.destroy', $container_check_reason->id) }}"
