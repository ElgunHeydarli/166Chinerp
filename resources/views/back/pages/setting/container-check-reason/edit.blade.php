...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Konteyner yoxlama səbəbi redaktə et</h2>
        <form method="POST" action="{{ route('admin.container-check-reason.update', $about_booking_date->id) }}"
            ...
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $container_check_reason->name }}"
                        placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">Status<span>*</span></label>
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $container_check_reason->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $container_check_reason->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
