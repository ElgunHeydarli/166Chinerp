...
@section('content')
    <div class="setting_content">
        <h2 style="font-size: 28px; margin-bottom:25px;">Mənbələr</h2>
        <div class="tabContent-head">
            <a href="{{ route('admin.source.create') }}" class="addSettingLink">
                    ...
                    <tbody>
                        @foreach ($sources as $source)
                            <tr data-id="{{ $source->id }}">
                                ...
                                <td>
                                    <label class="switch">
                                        <input type="checkbox" onchange="change_status(this,{{ $source->id }},'source')" {{ $source->status ? 'checked' : '' }}>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>{{ $source->created_at?->format('d.m.Y') ?? 'Yoxdur' }}</td>
                                <td>
                                    <div class="settingTable-operation">
                                        ...
                                        <div class="settingTable-operation-links">
                                            <a href="{{ route('admin.source.edit', $source->id) }}"
                                            ...
                                            <button data-href="{{ route('admin.source.destroy', $source->id) }}"
