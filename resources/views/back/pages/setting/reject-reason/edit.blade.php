...
@section('content')
    <div class="create-draftOrder-container">
        <form method="POST" action="{{ route('admin.reject-reason.update', $reject_reason->id) }}" class="create-draftOrder">
            @csrf
            @method('PUT')
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" name="name" value="{{ $reject_reason->name }}" placeholder="Text here...">
                </div>
                <div class="form-item">
                    <label for="">
                        Status<span>*</span>
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $reject_reason->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $reject_reason->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
