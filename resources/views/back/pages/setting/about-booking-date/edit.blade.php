...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="font-size: 28px; margin-bottom:25px;">Təxmini rezervasiya tarixi redaktə et</h2>
        <form method="POST" action="{{ route('admin.about-booking-date.update', $about_booking_date->id) }}"
            ...
            <div class="draftOrder-form">
                <div class="form-item">
                    ...
                    <input type="text" class="datetimepicker" autocomplete="off" name="date"
                        value="{{ $about_booking_date->date->format('d.m.Y') }}" placeholder="Tarix">
                ...
                <div class="form-item">
                    ...
                    <select name="status" class="nice-select" id="">
                        <option value="1" {{ $about_booking_date->status == 1 ? 'selected' : '' }}>Aktiv</option>
                        <option value="0" {{ $about_booking_date->status == 0 ? 'selected' : '' }}>Deaktiv</option>
                    </select>
                </div>
            </div>
