...
@section('content')
    <div class="create-draftOrder-container">
        <h2 style="margin-bottom:25px; font-size:28px;">Daşınma əlavə et</h2>
        <form method="POST" action="{{ route('admin.transportation.store') }}" class="create-draftOrder">
            @csrf
                    ...
                    <select name="transportation_service_id" id="">
                        ...
                        @foreach ($transportation_services as $transportation_service)
                            <option value="{{ $transportation_service->id }}"
                                {{ old('transportation_service_id') == $transportation_service->id ? 'selected' : '' }}>
                                {{ $transportation_service->name . ' (' . $transportation_service->transporation_type->name . ')' }}
                            </option>
                        @endforeach
                    </select>
                </div>
