<table>
    ...
    <tbody>
        @foreach ($container_prices as $container_price)
            <tr>
                <td>{{ $container_price->container_type->name . ' (' . $container_price->container_type->max_size . ')' }}
                </td>
                <td>{{ $container_price->station->name }}</td>
                <td>{{ $container_price->price ?? 0 }}</td>
                <td>
                    <div class="reservationPrice-buttons">

                        <button class="editReservationPrice"
                            data-action="{{ route('admin.container-price.update', $container_price->id) }}"
                            data-id="{{ $container_price->id }}" onclick="get_data(this)" type="button">
                        ...
                        <button class="deleteReservationPrice settingTable_delete"
                            data-href="{{ route('admin.container-price.destroy', $container_price->id) }}"
                            onclick="open_delete_modal(this)" type="button">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
...
{{ $container_prices->withQueryString()->links('back.section.pagination') }}
