...
@section('content')
    <div class="containerReservation-inner">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        @can('Konteyner rezervasiyası page-Bax-Status')
                            <th>Status</th>
                        @endcan
                        @can('Konteyner rezervasiyası page-Bax-Ödə')
                            <th>&nbsp;</th>
                        @endcan
                    </tr>
                </thead>
                <tbody>
                    @foreach ($booking_dates as $booking_date)
                        <tr>
                            <td>{{ $booking_date->date->format('d/m/Y') }}</td>
                            <td>{{ $booking_date->container_type->name . ' (' . $booking_date->container_type->max_size . ')' }}
                            </td>
                            <td>{{ $booking_date->station->name }}</td>
                            <td>{{ $booking_date->count ?? ' - ' }}</td>
                            <td>{{ $booking_date->price ?? ' - ' }}</td>
                            <td>{{ $booking_date->total_price ?? ' - ' }}</td>
                            <td>{{ $booking_date->remainder ?? ' - ' }}</td>
                            <td>{{ $booking_date->last_payment_date?->format('d/m/Y') ?? '-' }}</td>
                            @can('Konteyner rezervasiyası page-Bax-Status')
                                <td>
                                    <label class="switch">
                                        <input type="checkbox" onchange="change_booking_status(this)"
                                            data-id="{{ $booking_date->id }}" {{ $booking_date->status == 1 ? 'checked' : '' }}>
                            ...
                            @can('Konteyner rezervasiyası page-Bax-Ödə')
                                <td>
                                    <button class="paymetBtn" onclick="pay({{ $booking_date->id }})"
                                        type="button">Ödə</button>
                                </td>
                            @endcan
                        </tr>
                    @endforeach
    ...
    @include('back.pages.booking-container.section.payment')
@endsection

@push('js')
    <script>
        function change_booking_status(item) {
            let id = item.getAttribute('data-id');
            let url = `/booking-container/${id}/change-status`;
        ...
        function pay(id) {
            let form = document.querySelector('.payment_modal_container form');
            let action = `/booking-container/${id}/pay`;
