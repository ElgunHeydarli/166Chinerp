    ...
    <div class="finance_tab_content">
        <div class="tabContent-head">
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" autocomplete="off" name="start_date"
                    value="{{ request('start_date') }}" onchange="filter()" placeholder="Start Time">
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" autocomplete="off" name="end_date"
                    value="{{ request('end_date') }}" onchange="filter()" placeholder="End Time">
                <div class="calendar-icon">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
            ...
            <a href="{{ route('admin.salary-management.create') }}" class="addBulkPaymentLink">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 12H12M12 12H6M12 12V6M12 12V18" stroke="white" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round"></path>
                </svg>
                Toplu ödəniş əlavə et
            </a>
        </div>
        <div class="finance_tabContent_inner">
            @include('back.pages.account.section.tab-header')
            <div class="empPayHistoryTable">
                @include('back.pages.account.payment-history.section.filter', [
...
@push('js')
    <script>
        function open_operations(item) {
            ...
            if (!item.classList.hasClass('active')) item.classList.add('active');
            else item.classList.remove('active');
        }

        function close_all_operations() {
            document.querySelectorAll('.employee-operation-links').forEach(operation_link => {
                operation_link.classList.remove('active');
            });
        }
    </script>
    ...
    <script>
        function filter(page = 1) {
            ...
            let newUrl = `/payment-history?${params.toString()}`;
            let url = `/payment-history/filter?${params.toString()}`;
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.status == 'success') {
                        let empPayHistoryTable = document.querySelector('.empPayHistoryTable');
                        empPayHistoryTable.innerHTML = data.view;
                    }
                });
        }
    </script>
