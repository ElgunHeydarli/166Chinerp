...
@section('content')
    <div class="finance_tab_content">
        <div class="tabContent-head">
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" name="start_date" value="{{ request('start_date') }}"
                    onchange="filter()" autocomplete="off" placeholder="Start Time" />
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" autocomplete="off" placeholder="End Time" name="end_date"
                    value="{{ request('end_date') }}" onchange="filter()" />
                <div class="calendar-icon">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
        ...
        <div class="finance_tabContent_inner">
            ...
            <div class="receivable_pay_history">
                @include('back.pages.account.receive.section.history', [
...
@push('js')
    <script>
        function filter(page = 1) {
            ...
            let newUrl = `/receive/history?${params.toString()}`;
            let url = `/receive/history/filter?${params.toString()}`;
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.status == 'success') {
                        document.querySelector('.receivable_pay_history').innerHTML = data.view;
                    }
                });
        }
    </script>
