...
@section('content')
    <div class="editReceivable-container">
        ...
        <form action="{{ route('admin.receive.update', $receive->id) }}" method="POST" class="editReceivable-form">
            @csrf
            @method('PUT')
            <div class="form-line">
                <div class="form-item">
                    <label for=""> İnvoice İD </label>
                    <input type="text" name="invoice_id" oninput="convert_alphanumeric(this)"
                        value="{{ old('invoice_id', $receive->invoice_id) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for="">LOG</label>
                    <select name="order_id" id="" class="select2">
                        <option value="">Seçim edin</option>
                        @foreach ($orders as $order)
                            <option value="{{ $order->id }}"
                                {{ old('order_id', $receive->order_id) == $order->id ? 'selected' : '' }}>
                                {{ $order->code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-item">
                    <label for=""> Müştəri adı </label>
                    <select name="customer_id" class="nice-select">
                        <option value="">Seçin</option>
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->id }}"
                                {{ old('customer_id', $receive->customer_id) == $customer->id ? 'selected' : '' }}>
                                {{ $customer->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-item">
                    <label for=""> Satılan məhsul/xidmət </label>
                    <input type="text" name="service_name" oninput="convert_letter(this)"
                        value="{{ old('service_name', $receive->service_name) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> İnvoice tarixi </label>
                    <input type="text" name="invoice_date"
                        value="{{ old('invoice_date', $receive->invoice_date?->format('d.m.Y')) }}" class="datetimepicker"
                        autocomplete="off" placeholder="Tarix" />
                ...
                <div class="form-item">
                    <label for=""> Son ödəmə üçün möhlət </label>
                    <input type="text" name="last_payment_date"
                        value="{{ old('last_payment_date', $receive->last_payment_date?->format('d.m.Y')) }}"
                        class="datetimepicker" autocomplete="off" placeholder="Tarix" />
                    <div class="calendar-icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                    ...
                    <select name="country_id" id="" class="select2">
                        ...
                        @foreach ($countries as $country)
                            <option value="{{ $country->id }}"
                                {{ old('country_id', $receive->country_id) == $country->id ? 'selected' : '' }}>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="currency" id="" class="nice-select">
                        ...
                        @foreach ($currencies as $currency)
                            <option value="{{ $currency->code }}"
                                {{ old('currency', $receive->currency) == $currency->code ? 'selected' : '' }}>
                                {{ $currency->code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-item">
                    ...
                    <input type="text" name="price" oninput="calculate_total_price(this)"
                        value="{{ old('price', $receive->price) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> ƏDV </label>
                    <input type="text" name="vat" oninput="calculate_total_price(this)"
                        value="{{ old('vat', $receive->vat) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> Cəmi məbləğ </label>
                    <input type="text" name="total_price" oninput="convert_numeric(this)"
                        value="{{ old('total_price', $receive->total_price) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> Avans ödənişi </label>
                    <input type="text" name="initial_payment" oninput="calculate_remainder(this)"
                        value="{{ old('initial_payment', $receive->initial_payment) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> Qalıq məbləğ </label>
                    <input type="text" name="remainder" oninput="convert_numeric(this)"
                        value="{{ old('remainder', $receive->remainder) }}" placeholder="text here" />
                </div>
                <div class="form-item">
                    <label for=""> Ödəniş statusu </label>
                    <select name="status" id="" class="nice-select">
                        <option value="{{ \App\Enums\ReceiveStatus::PAID }}"
                            {{ old('status', $receive->status) == \App\Enums\ReceiveStatus::PAID ? 'selected' : '' }}>
                            {{ \App\Enums\ReceiveStatus::PAID->label() }}</option>
                        <option value="{{ \App\Enums\ReceiveStatus::PENDING }}"
                            {{ old('status', $receive->status) == \App\Enums\ReceiveStatus::PENDING ? 'selected' : '' }}>
                            {{ \App\Enums\ReceiveStatus::PENDING->label() }}</option>
                        <option value="{{ \App\Enums\ReceiveStatus::NOT_PAID }}"
                            {{ old('status', $receive->status) == \App\Enums\ReceiveStatus::NOT_PAID ? 'selected' : '' }}>
                            {{ \App\Enums\ReceiveStatus::NOT_PAID->label() }}</option>
                    </select>
                    <button class="resetSelectBtn" type="button">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="submitEditReceivable" type="submit">Yadda saxla</button>
        </form>
    </div>
@endsection
...
@push('js')
    ...
    <script>
        function calculate_remainder(item) {
            convert_numeric(item);
            let initial_payment_value = item.value;
            let total_price_value = document.querySelector('[name="total_price"]').value ?? 0;
            let remainder = document.querySelector('[name="remainder"]');
            remainder.value = +total_price_value - +initial_payment_value;
        }

        function calculate_total_price(item) {
            convert_numeric(item);
            let price_value = document.querySelector('[name="price"]').value ?? 0;
            let vat_value = document.querySelector('[name="vat"]').value ?? 0;
            let total_price = document.querySelector('[name="total_price"]');
            let total_price_value = +price_value + +(price_value * vat_value / 100);
            total_price.value = total_price_value.toFixed(2);
        }
    </script>
@endpush
