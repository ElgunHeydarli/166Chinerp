<div class="receivableTable-table">
    <table>
        <thead>
            <tr>
                <th style="white-space: normal;">Invoice ID</th>
                <th style="white-space: normal;">LOG</th>
                <th style="white-space: normal;">Müştəri adı</th>
                <th style="white-space: normal;">Satılan xidmət/məhsul</th>
                <th style="white-space: normal;">İnvoice tarixi</th>
                <th style="white-space: normal;">Son ödəmə üçün möhlət</th>
                <th style="white-space: normal;">Valyuta</th>
                <th style="white-space: normal;">Məbləğ</th>
                <th style="white-space: normal;">ƏDV</th>
                <th style="white-space: normal;">Cəmi məbləğ</th>
                <th style="white-space: normal;">Avans ödənişi</th>
                <th style="white-space: normal;">Qalıq məbləğ</th>
                <th style="white-space: normal;">Ödəniş statusu</th>
        ...
        <tbody>
            @foreach ($receives as $receive)
                <tr>
                    <td><a href="{{ route('admin.receive.show', $receive->id) }}">{{ $receive->invoice_id }}</a></td>
                    <td>{{ $receive->order?->code ?? ' - ' }}</td>
                    <td>{{ $receive->customer?->name ?? ' - ' }}</td>
                    <td>{{ $receive->service_name ?? ' - ' }}</td>
                    <td>{{ $receive->invoice_date?->format('d.m.Y') ?? ' - ' }}</td>
                    <td>{{ $receive->last_payment_date?->format('d.m.Y') ?? ' - ' }}</td>
                    <td>{{ $receive->currency ?? ' - ' }}</td>
                    <td>{{ $receive->price ?? ' - ' }}</td>
                    <td>{{ $receive->vat ?? ' - ' }}</td>
                    <td>{{ $receive->total_price ?? ' - ' }}</td>
                    <td>{{ $receive->initial_payment ?? ' - ' }}</td>
                    <td>{{ $receive->remainder ?? ' - ' }}</td>
                    <td>
                        <div class="receivableStatus {{ $receive->status?->class_name() }}">
                            <span></span>
                            <p>{{ $receive->status->label() }}</p>
                        </div>
                    </td>
                    <td>
                        <div class="receivableTable-operation">
                            <button class="receivableTable-operation-btn" type="button">
                                Əməliyyat
                            ...
                            <div class="receivableTable-operation-links">
                                <a href="{{ route('admin.receive.show', $receive->id) }}"
                                    class="receivableTable-operation-link receivableTable-view">
                                ...
                                <a href="{{ route('admin.receive.edit', $receive->id) }}"
                                    class="receivableTable-operation-link receivableTable-edit">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
...
{{ $receives->withQueryString()->links('back.section.pagination') }}
