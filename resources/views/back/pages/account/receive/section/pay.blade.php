<div class="addReceivablePayModal_container">
    <div class="addReceivablePayModal">
        <h2>Ödəniş əlavə et</h2>
        <button class="closeAddReceivablePayModal" type="button">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18 6L6 18M6 6L18 18" stroke="black" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round" />
            </svg>
        </button>
        <form action="{{ route('admin.receive.pay', $receive->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-line">
                <div class="form-item">
                    <label for=""> Tarix </label>
                    <input type="text" class="datetimepicker" name="date" value="{{ old('date') }}"
                        autocomplete="off" />
                ...
                <div class="form-item">
                    ...
                    <select name="payment_method" id="" class="nice-select">
                        <option value="{{ \App\Enums\PaymentMethod::BANK }}"
                            {{ old('payment_method') == \App\Enums\PaymentMethod::BANK ? 'selected' : '' }}>Bank
                        </option>
                        <option value="{{ \App\Enums\PaymentMethod::CASH }}"
                            {{ old('payment_method') == \App\Enums\PaymentMethod::CASH ? 'selected' : '' }}>Nağd
                        </option>
                    </select>
                    <button class="resetSelectBtn" type="button">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
            ...
            <div class="form-item">
                ...
                <textarea name="note" id="" placeholder="Note">{{ old('note') }}</textarea>
            </div>
            <div class="addReceivableFile">
                <div class="icon">
                ...
                <div class="addReceivableFile-body">
            ...
            <div class="addReceivableFile-fileUpload-area">
                <img src="{{ asset('back/assets/images/pdfImg.svg') }}" alt="" />
                <div class="addReceivableFile-fileUpload-main">
                    <div class="addReceivableFile-fileUpload-top">
                        <span class="addReceivableFile-FileName"></span>
                        <p class="addReceivableFile-fileSize"></p>
                    </div>
                    <div class="addReceivableFile-fileProgress">
                        <div class="uploadLine"></div>
                    </div>
                </div>
                <button class="removeAddReceivableFile" type="button">
            ...
            <button class="save_addReceivablePay" type="submit">Save</button>
        </form>
    </div>
</div>
