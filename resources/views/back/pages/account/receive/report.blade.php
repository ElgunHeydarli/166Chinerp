    ...
    <div class="finance_tab_content">
        <div class="tabContent-head">
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" autocomplete="off" placeholder="Start Time" name="start_date"
                    value="{{ request('start_date') }}" oninput="filter()" />
            ...
            <div class="filterColumns">
                <input type="text" class="datetimepicker" placeholder="End Time" autocomplete="off" name="end_date"
                    value="{{ request('end_date') }}" oninput="filter()" />
                <div class="calendar-icon">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
        ...
        <div class="finance_tabContent_inner">
            ...
            <div class="receivable_history">
                @include('back.pages.account.receive.section.report-filter')
            </div>
        </div>
    </div>
@endsection
...
@push('js')
    <script>
        function filter(page = 1) {
            ...
            let newUrl = `/receive/report?${params.toString()}`;
            let url = `/receive/report/filter?${params.toString()}`;
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.status == 'success') {
                        document.querySelector('.receivable_history').innerHTML = data.view;
                    }
                });
        }
    </script>
