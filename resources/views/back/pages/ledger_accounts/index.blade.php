...
@section('title', 'Hesablar cədvəli')

@section('content')
    <script src="https://cdn.tailwindcss.com"></script>

    <div class="finance_tab_content">
        <div class="tabContent-head flex flex-wrap items-center justify-between gap-4 mt-4">
            <!-- Filtr formu -->
            <form action="" method="GET" id="filterForm" class="flex flex-wrap items-center gap-3">
                <!-- Axtarış inputu -->
                <input type="text" name="search" placeholder="Axtar..." value="{{ request('search') }}"
                    class="border border-gray-300 rounded px-4 py-2 w-48 focus:outline-none focus:ring-2 focus:ring-blue-500" />

                <!-- Hesab növü filteri -->
                <select name="type"
                    class="border border-gray-300 rounded px-3 py-2 h-10 w-48 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">Hesab növü</option>
                    <option value="aktiv" {{ request('type') == 'aktiv' ? 'selected' : '' }}>Aktiv</option>
                    <option value="passiv" {{ request('type') == 'passiv' ? 'selected' : '' }}>Passiv</option>
                    <option value="xərc" {{ request('type') == 'xərc' ? 'selected' : '' }}>Xərc</option>
                    <option value="gəlir" {{ request('type') == 'gəlir' ? 'selected' : '' }}>Gəlir</option>
                </select>

                <!-- Filtrlə düyməsi -->
                <button type="submit"
                    class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded transition duration-150 flex items-center gap-2">
            ...
            <a href="{{ route('admin.ledger-accounts.create') }}"
                class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded flex items-center gap-2 transition duration-150">
                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5"
                    stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
        ...
        <div class="accountTable mt-4">
            <div class="accountTable-table">
                <table>
                    <thead>
                        <tr>
                            <th>Hesabın nömrəsi</th>
                            <th>Hesabın adı</th>
                            <th>Hesab növü</th>
                            <th>Qeyd</th>
                            <th>Status</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-{{ $account->status ? 'success' : 'secondary' }}">
                                        {{ $account->status ? 'Aktiv' : 'Deaktiv' }}
                                    </span>
                                </td>
                                <td>
                                    <div class="accountTable-operation">
                                        <button class="accountTable-operation-btn" type="button">
                                            Əməliyyat
                                            <svg width="19" height="19" fill="none" stroke="black"
                                                stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M5 7.39L9.5 11.89L14 7.39" />
                                            </svg>
                                        </button>
                                        <div class="accountTable-operation-links">
                                            <a href="{{ route('admin.ledger-accounts.edit', $account->id) }}"
                                                class="accountTable-operation-link accountTable-edit">
                                            ...
                                            <form method="POST"
                                                action="{{ route('admin.ledger-accounts.destroy', $account->id) }}">
                                                @csrf
                                                @method('DELETE')

                                                <button type="submit" onclick="delete_item(this)"
                                                    class="accountTable-operation-link accountTable-edit">
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path
    ...
    <script>
        function delete_item(item) {
            ...
            let confirmed = confirm('Məlumatı silmək istədiyinizdən əminsiniz mi?');
            if (confirmed) {
